...
group.add_argument('--exl3-no-defrag', action='store_true', help='ExLlamaV3: disable periodic cache defragmentation.')
group.add_argument('--exl3-moe-cpu-layers', type=int, default=0, help='ExLlamaV3: run the routed experts of the first N MoE layers on the CPU. Layer-split mode only.')
...
group.add_argument('--exl3-ngram-match-min', type=int, default=0, help='ExLlamaV3: minimum match length for n-gram drafting. 0 = disabled. Ignored when a draft model or MTP head is used.')
group.add_argument('--exl3-dynamic-draft', action='store_true', help='ExLlamaV3: adapt the draft length to the acceptance rate, with draft-max as the ceiling.')
group.add_argument('--exl3-draft-confidence', type=float, default=0.4, help='ExLlamaV3: confidence target for dynamic draft truncation.')
group.add_argument('--exl3-layer-map', type=str, default=None, help='ExLlamaV3: RYS layer map as a list of ints or inclusive ranges. Example: 0..15,11..31')
group.add_argument('--exl3-override', type=str, default=None, help='ExLlamaV3: path to a YAML tensor override spec.')
group.add_argument('--exl3-load-verbose', action='store_true', help='ExLlamaV3: verbose output while loading.')
